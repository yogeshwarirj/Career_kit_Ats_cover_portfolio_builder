...
const parseTextToResume = (text: string): ParsedResume => {
  ...
  const projectsIndex = lines.findIndex(line => 
    /projects|portfolio/i.test(line)
  );
  if (projectsIndex !== -1) {
    const projectLines = lines.slice(projectsIndex + 1, Math.min(projectsIndex + 10, lines.length));
    projectLines.forEach((line, index) => {
      if (line.length > 10 && !line.match(/^(experience|education|skills|certifications)/i)) {
  ...
  const certificationsIndex = lines.findIndex(line => 
    /certifications|certificates|licenses/i.test(line)
  );
  if (certificationsIndex !== -1) {
    const certLines = lines.slice(certificationsIndex + 1, Math.min(certificationsIndex + 8, lines.length));
    certLines.forEach((line, index) => {
      if (line.length > 5 && !line.match(/^(experience|education|skills|projects)/i)) {
        resume.certifications.push({
          id: Date.now().toString() + index,
          name: line.split('-')[0]?.trim() || line,
          issuer: line.split('-')[1]?.trim() || 'Issuer',
          date: line.match(/\d{4}/)?.[0] || ''
        });
      }
    });
  }
  ...
  // Extract languages
  const languagesIndex = lines.findIndex(line => 
    /languages|language skills/i.test(line)
  );
  if (languagesIndex !== -1) {
    const langLines = lines.slice(languagesIndex + 1, Math.min(languagesIndex + 5, lines.length));
    langLines.forEach((line, index) => {
      if (line.length > 3 && !line.match(/^(experience|education|skills|projects|certifications)/i)) {
        resume.languages?.push({
          id: Date.now().toString() + index,
          name: line.split('-')[0]?.trim() || line,
          proficiency: line.split('-')[1]?.trim() || 'Conversational'
        });
      }
    });
  }
